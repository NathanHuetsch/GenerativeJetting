...
def plot_obs(pp, obs_train, obs_test, obs_predict, name, bins=60, range=None, unit=None, weight_samples=1,
             error_range = [0.71, 1.29], predict_weights=None, n_epochs=None, n_jets=None, y_ticks=[0.8,1,1.2], lab = ['CFM','CM','Train']):
    '''
    ...
    with warnings.catch_warnings():
        ...
        if weight_samples == 1:
            ...
            obs_predict = obs_predict.reshape(weight_samples, len(obs_predict)//weight_samples)
        ...
        axs[1].set_ylabel(r"$\frac{\mathrm{model}}{\mathrm{Train}}$", fontsize = FONTSIZE)
        ...
        plt.xlabel(r"${%s}$ %s" % (name, ("" if unit is None else f"[{unit}]")), fontsize = FONTSIZE)
